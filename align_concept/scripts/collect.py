...
parser.add_argument('--prefix_fasta')
parser.add_argument('--prefix_idx')
...
for i in range(args.nrepeat):
    ...
    out_fasta = open('{prefix}_{i}.fa'.format(prefix = args.prefix_fasta, i = i), 'w')
    for j in range(len(indice)):
        idx = indice[j]
        seq = my_python.binary_to_char(x_subset[j])
        out_fasta.write('>{name}\n{seq}\n'.format(name = idx, seq = seq))
    out_fasta.close()
    out_text = open('{prefix}_{i}.txt'.format(prefix = args.prefix_idx, i = i), 'w')
