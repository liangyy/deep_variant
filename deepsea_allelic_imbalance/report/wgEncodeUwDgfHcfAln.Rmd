...
library(ggplot2)
library(reshape2)
variant_url <- 'data/wgEncodeUwDgfHmvecdbladAln.imbalance_pvalue.gz'
variant <- read.table(variant_url, sep = '	', header = T)
score_url <- '../../DeepVariantPrediction/score/keras_deepsea_copy/allelic_imbalance_wgEncodeUwDgfHmvecdbladAln.wgEncodeUwDgfHmvecdbladAln_allelic_imbalance_result.feather'
...
ggplot(variant) + geom_point(aes(x = Alt, y = Ref, color = log10(Odds.Ratio))) + facet_grid(.~Imbalance) + geom_abline(intercept = 0.07, slope = 1) + geom_abline(intercept = -0.07, slope = 1) + ggtitle('The Predicted Effect of Heterozygous sites in DNase-seq of {name}') + scale_color_gradient2(midpoint=0, low="blue", mid="white", high="red", space ="Lab" )
```
