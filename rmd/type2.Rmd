...
sdg <- read.table('../data/type2_040717_00_04_09_19.csv', sep = ',', header = T)
...
```{r}
performance_cross_epoch <- function(scores, deepsea.aucs){
    ...
    p1 <- ggplot(aucs.plot[aucs.plot$variable == 'roc_auc',]) + geom_histogram(aes(x = value, fill = Annotation.Type)) + facet_grid(epoch~Annotation.Type) + ggtitle('SGD - ROC AUC')
    p2 <- ggplot(aucs.plot[aucs.plot$variable == 'pr_auc',]) + geom_histogram(aes(x = value, fill = Annotation.Type)) + facet_grid(epoch~Annotation.Type) + ggtitle('SGD - PR AUC')
    return(list(roc=p1, pr=p2, data=aucs.plot))
}
re <- performance_cross_epoch(sdg, deepsea.aucs)
re$roc
re$pr
re <- performance_cross_epoch(rmsprop, deepsea.aucs)
re$roc
re$pr
```
